...
public class psMessages {
    ...
    public static void CostMess(Player p, int price, Vector size, int summSize)
    {
        ...
        p.sendMessage(ChatColor.YELLOW  + "" + ChatColor.BOLD + "PrivateSeller" + ChatColor.GREEN + ": "
                ...
                + ChatColor.YELLOW + " : " + (int)size.getX() + " " + (int)size.getY() + " " + (int)size.getX());
        p.sendMessage(ChatColor.YELLOW + "PrivateSeller"+ "" + ChatColor.BOLD + ChatColor.GREEN + ": " + ChatColor.YELLOW + " Стоимость региона составляет : " + ChatColor.GREEN + "" + ChatColor.BOLD + price + ChatColor.YELLOW + "$");
